...
int read_remote(char *dest) {
  ...
  if (REMOTE_PIN == 1) {
    ...
    if (!(TCNT1 > 10000 && TCNT1 < 22000))
      return; // if preamble signal wasn't recognized, just ignore the rest and
              // return
  ...
  while (1) {
    ...
    if (status_obj.is_locked != 1) {
      ...
      if (REMOTE_LEARN_PIN == 1) {
        ...
        // eeprom_write_block(remote_code, REMOTE_CODE_EEPROM_ADDRESS,
        // sizeof(remote_code));
      } else {
        LEARN_ENABLED_LED = 0;
      }
